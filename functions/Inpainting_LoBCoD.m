function [cleanI,objective,avgpsnr,sparsity,totTime,alpha,D] = Inpainting_LoBCoD(params,TV)
...
%              TV='TVCorrection'
         if TV
           ...
           x_low=imresize(x,[cleanI_x cleanI_y],'nearest');
           cleanI{i}=TVCorrection(x_low,2,1);
           cleanI{i}=imresize(cleanI{i},[N1,N2],'nearest'); 
           cleanI{i}(1,:)= x(1,:);
           cleanI{i}(:,1)= x(:,1); 
         end
           %NAN Ìæ´ú³É0
         cleanI{i}(find(isnan( cleanI{i})==1)) = 0;
         noisyImean{i}(find(isnan(  noisyImean{i})==1)) = 0;
         lmnI{i}(find(isnan(  noisyImean{i})==1)) = 0;
         I{i}(find(isnan(  noisyImean{i})==1)) = 0;
         noisyI{i}(find(isnan(  noisyImean{i})==1)) = 0;
         M{i}(find(isnan(  noisyImean{i})==1)) = 0;
 
%         figure,imagesc(cleanI{1}+noisyImean{1}),colormap('gray'),axis off
%         final = cleanI{1}+noisyImean{i};
%         final(find(cleanI{1}+noisyImean{i}< 1000)) = 0;
%         figure,imagesc((final))
        
        max_Number=max(max(cleanI{i}+noisyImean{i}));
        sumpsnr = sumpsnr + 20*log10((max_Number*sqrt(numel(I{i}))) / norm(reshape((cleanI{i}+noisyImean{i}) - (I{i}+lmnI{i}),1,[])));
...
%         a=imresize(alphaN{1}(1,:),[427,561]);       
%         figure,imshow(a,[-0.5,0.5])
%         showDictionary(D);
%         figure,imagesc(b(1:8,1:8)),colormap gray;
